...
Modular arithmetic primitives on `qint_t` (`add_mod`, `mul_mod`, `pow_mod`, and the transpiler-folded forms `(a + b) % n`, `(a * b) % n`, `pow(a, x) % n`) require that all operands satisfy the precondition `a, b ∈ [0, n)` (and for `pow_mod`, the base `a ∈ [0, n)`; the exponent `x` may be any non-negative integer up to the register width). The library does not check this precondition at runtime; supplying out-of-range operands is undefined behavior, in the same trust model as classical C/C++ modular reduction. See `docs/archive/prd_modular_arithmetic.md` for the full specification, the rationale for omitting the check, and the per-primitive contract details.
