...
> - PL-6 (sturm-pumi): tag `v0.1.0` pushed; source tarball sha256 `14674d0c146e41bb9c050d8247358b0cc8fd39ba7861af4ca0ba7af2a6e8a7ef` committed into `github.com/SoerenWilkening/homebrew-sturm` at `Formula/sturm-transpile.rb`. Release page: `https://github.com/SoerenWilkening/Sturm_CPP/releases/tag/v0.1.0`. Initial tag pushed at `d971184`; release workflow OOM-killed both Linux builds with `cmake --build -j` (unlimited parallelism on 2-core runners); fix lands in `99d96ad` (`-j 2`) and the tag was force-deleted + re-created at that commit. The source-tarball sha256 above is the post-retag value.
