// test_div_when_lift.cpp -- sturm-a3t4.4 P3
//
// Pins the depth-1 lift pattern in `lib_div_dsl` (forward + adjoint
// kernels, both share `detail_div::div_kernel<Bit, Forward>`).  The
// kernel replaced the inner push_ov / push_sgn_i / push_q0 lambdas (and
// their pop_control bookends) with the inline lift:
//
...
// body must execute under a single control qubit regardless of whether
// the caller wrapped the division in an enclosing `WHEN(c)`.
//
// Coverage rationale (issue body): "reduce surface as in P1/P2 — just
// enough cases for correctness."  Exhaustive sweeps in
// tests/lib/test_div_mod_dsl_adjoint.cpp already cover correctness across
// (a, b); this test focuses on the new control-stack lift specifically.
//
// Method per (a, b) input (mirrors test_mul_mod_when_lift.cpp's
// APPEND-mode classical replay harness):
//   1. Run lib_div_dsl in APPEND mode, capturing X/CX/CCX into ctx.ir.
//   2. Replay the stream as a classical bit-flip program seeded with
//      (a, b).
//   3. Assert q == a/b, r == a%b, inputs unchanged, pool live-count
//      restored.
...
static void run_case(uint32_t a_val, uint32_t b_val, bool wrap_in_when) {
    assert(b_val != 0u && "test precondition: divisor non-zero");

    sturm::QubitPool::instance().reset_for_testing();

    int qi_a[W], qi_b[W], qi_q[W], qi_r[W];
    for (std::size_t i = 0; i < W; ++i)
        qi_a[i] = sturm::QubitPool::instance().allocate();
    for (std::size_t i = 0; i < W; ++i)
        qi_b[i] = sturm::QubitPool::instance().allocate();
    for (std::size_t i = 0; i < W; ++i)
        qi_q[i] = sturm::QubitPool::instance().allocate();
    for (std::size_t i = 0; i < W; ++i)
    ...
    sturm_backend_context_t* ctx =
        sturm_backend_create(STURM_MODE_APPEND, 256u);
    assert(ctx);
    ...
    if (wrap_in_when) {
        WHEN(c_owner) {
            sturm::lib_div_dsl<sturm::BitProxy>(a_bits, W, b_bits, W,
                                                 q_bits, r_bits);
        }
    } else {
        sturm::lib_div_dsl<sturm::BitProxy>(a_bits, W, b_bits, W,
    ...
    static_assert(adj_ptr != nullptr,
                  "invert<&lib_div_dsl<BitProxy>>() must resolve");
    ...
    if (wrap_in_when) {
        WHEN(c_owner) {
            adj_ptr(a_bits, W, b_bits, W, q_bits, r_bits);
        }
    } else {
        adj_ptr(a_bits, W, b_bits, W, q_bits, r_bits);
    }
    ...
    // Replay the forward portion to verify intermediate q == a/b, r == a%b.
    {
        ...
        const uint32_t expect_q = a_val / b_val;
        ...
        assert(read_reg_classical(bits, qi_b, W) == b_val
               && "fwd: b preserved");
        assert(read_reg_classical(bits, qi_q, W) == expect_q
               && "fwd: q == a/b");
        assert(read_reg_classical(bits, qi_r, W) == expect_r
               && "fwd: r == a%b");
    }

    // Replay the full forward+adjoint sequence to verify q,r return to 0
    // and ancilla bits clean up.
    {
        ...
        assert(read_reg_classical(bits, qi_b, W) == b_val
               && "adj: b preserved");
        assert(read_reg_classical(bits, qi_q, W) == 0u
               && "adj: q returned to |0>");
    ...
    for (std::size_t i = W; i-- > 0;)
        sturm::QubitPool::instance().release(qi_q[i]);
    for (std::size_t i = W; i-- > 0;)
...
int main() {
    std::printf("sturm-a3t4.4: depth-1 lift pattern in lib_div_dsl "
                "(W=%zu, with/without enclosing WHEN(c)):\n", W);

    // Two operand pairs:
    //   - 3/2 = 1, rem 1 (exercises the final-correction add-back path
    //     because non-restoring division produces a tentatively-negative
    //     remainder before the q[0]-controlled add).
    //   - 2/3 = 0, rem 2 (exercises the divisor-larger-than-dividend
    //     branch: every q[i] stays 0).
    ...
    for (const Pair& t : cases) {
        ...
        std::printf("  PASS: bare WHEN(overflow|sgn[i]|q[0]) "
    ...
    std::printf("All sturm-a3t4.4 div lift-pattern tests passed.\n");
    return 0;
}
