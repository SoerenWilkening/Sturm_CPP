# check_quantum_to_classical_cond_diagnostic.cmake — PM3-5 Class 2 probe.
#
# Clones the PM3-4 `check_when_operand_mutation_diagnostic.cmake` pattern:
...
#                                    quantum_to_classical_cond_input.cpp
#                                    (positive) or
#                                    quantum_to_classical_cond_clean.cpp
#                                    (negative)
...
#                                    diagnostic MUST cite.
#
# Contract (PM3-5 acceptance)
# ---------------------------
# Positive fixture (MUST_CONTAIN + EXPECTED_LINE):
#   - sturm-transpile exits NON-ZERO (Error severity — compilation must
#     abort, not continue).
#   - stderr contains MUST_CONTAIN (the locked-down PM3-5 format
#     fragment).
#   - stderr contains `<basename(SOURCE)>:<EXPECTED_LINE>:` — proving the
#     diagnostic cites the user's source file at the cast line.
#   - stderr does NOT contain `<memory-buffer>`.
#
...
foreach(var TRANSPILE_BIN SOURCE OUTPUT_DIR)
    if(NOT DEFINED ${var})
        message(FATAL_ERROR
            "check_quantum_to_classical_cond_diagnostic: ${var} not set")
    endif()
endforeach()

if(NOT DEFINED MUST_CONTAIN AND NOT DEFINED MUST_NOT_CONTAIN)
    message(FATAL_ERROR
        "check_quantum_to_classical_cond_diagnostic: neither "
        "MUST_CONTAIN nor MUST_NOT_CONTAIN was supplied — caller must "
        "pick exactly one.")
endif()
if(DEFINED MUST_CONTAIN AND DEFINED MUST_NOT_CONTAIN)
    message(FATAL_ERROR
        "check_quantum_to_classical_cond_diagnostic: both MUST_CONTAIN "
        "and MUST_NOT_CONTAIN were supplied — caller must pick exactly "
        "one.")
endif()
...
foreach(path "${TRANSPILE_BIN}" "${SOURCE}")
    if(NOT EXISTS "${path}")
        message(FATAL_ERROR
            "check_quantum_to_classical_cond_diagnostic: required file "
            "missing: ${path}")
    endif()
endforeach()
...
# not an absolute build-tree path.
execute_process(
...
if(DEFINED MUST_CONTAIN)
    ...
    # Contract #1 (positive): non-zero exit.
    if(_transpile_rc EQUAL 0)
        message(FATAL_ERROR
            "check_quantum_to_classical_cond_diagnostic: sturm-transpile "
            "exited zero on the positive fixture. The PM3-5 matcher "
    ...
    # Contract #2 (positive): negative `<memory-buffer>` check.
    string(FIND "${_transpile_err}" "<memory-buffer>" _memory_buf_pos)
    if(NOT _memory_buf_pos EQUAL -1)
        message(FATAL_ERROR
            "check_quantum_to_classical_cond_diagnostic: stderr "
            "references `<memory-buffer>` — the PM3-0 `DiagContext` "
            "lost the user's filename when converting the matched "
            "loc.\nstderr:\n${_transpile_err}")
    endif()
    ...
    if(_pos EQUAL -1)
        message(FATAL_ERROR
            "check_quantum_to_classical_cond_diagnostic: stderr did "
            "NOT contain the required substring `${MUST_CONTAIN}`.\n"
            "stderr:\n${_transpile_err}")
    endif()

    # Contract #4 (positive; optional): EXPECTED_LINE.
    if(DEFINED EXPECTED_LINE)
        ...
        if(_found_loc EQUAL -1)
            message(FATAL_ERROR
                "check_quantum_to_classical_cond_diagnostic: stderr "
                "does not cite the expected user-source location.\n"
    ...
    message(STATUS
        "check_quantum_to_classical_cond_diagnostic: OK — stderr "
        "contained `${MUST_CONTAIN}`, exit code was non-zero, and "
        "`<memory-buffer>` did not surface.")
else()
    # Negative branch: expect zero exit AND substring absent.

    # Contract #1 (negative): zero exit.
    if(NOT _transpile_rc EQUAL 0)
        message(FATAL_ERROR
            "check_quantum_to_classical_cond_diagnostic: sturm-transpile "
            "exited non-zero (rc=${_transpile_rc}) on the negative "
            "fixture. The PM3-5 matcher must not fire when the "
            "branch condition is classical or the control construct "
            "is WHEN's three-if tower.\n"
            "stdout:\n${_transpile_out}\n"
            "stderr:\n${_transpile_err}")
    endif()
    ...
    if(NOT _pos EQUAL -1)
        message(FATAL_ERROR
            "check_quantum_to_classical_cond_diagnostic: stderr "
            "contained the forbidden substring `${MUST_NOT_CONTAIN}` "
            "at offset ${_pos}. The matcher fired on a shape it "
            "should have rejected.\nstderr:\n${_transpile_err}")
    endif()

    message(STATUS
        "check_quantum_to_classical_cond_diagnostic: OK — stderr did "
        "NOT contain `${MUST_NOT_CONTAIN}` and exit code was zero.")
endif()
