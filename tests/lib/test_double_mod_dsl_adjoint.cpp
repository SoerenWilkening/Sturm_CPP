...
//
// Forward `lib_double_mod_dsl(x, n_bits, W)` followed by
// `__lib_double_mod_dsl_adj(x, n_bits, W)` must return `x_bits` to its
// original value for every (x, n) input that the forward beat covers
// (the W=2 exhaustive odd-n sweep, the (n+1)/2 LSB-edge case, plus a
// W=3 random sweep with the same odd-n restriction).  `n_bits` must
// remain unchanged across the full forward+adjoint pair, and
...
// the forward (not a literal halving).  The adjoint receives x_bits
// = 1 and must restore x_bits = 2 (=(n+1)/2).
//
// Mirrors `tests/lib/test_add_mod_dsl_adjoint.cpp` (sturm-yh3d.5):
//   (1) drive the forward,
//   (2) verify outputs match the classical reference,
...
//   (5) assert x_bits has been restored and n_bits is intact.
...
//
// Precondition: x < n, n odd, n >= 1.  Asserts:
...
static void run_roundtrip_case(uint32_t x_val, uint32_t n_val) {
    ...
    const uint32_t n_reg = (W + 1u) + W;  // x (W+1 slots), n (W slots)
    int reserved[n_reg];
    ...
    adj_ptr(x.bits.data(), n.bits.data(), W);
...
int main() {
    std::printf("sturm-wdas double-mod-dsl: adjoint round-trip "
    ...
    std::printf("  PASS: %zu W=2 cases (forward + adjoint restores x, "
                "preserves n, no leaked ancillas)\n", cases_run);
    ...
    std::puts("  PASS: round-trip on x=2, n=3 (forward 2·2 mod 3 = 1, "
              "LSB=1; adjoint restores x=2)");

    std::printf("All sturm-wdas double-mod-dsl adjoint tests passed.\n");
    return 0;
}
