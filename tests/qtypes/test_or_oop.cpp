// test_or_oop.cpp -- sturm-ph6f.1: sturm::detail::or_oop reversible-network tests.
//
// Contract: or_oop(a, b, tmp) emits a per-bit reversible OR ladder
// (`tmp_i ^= a_i | b_i`, dispatched into `lib_or_dsl`) when at least one
// operand carries allocated qubits, and falls back to the pure classical
// update when both operands are on the qubits[0] < 0 short-circuit path
// (preserves example_qint_arith / example_phase_abc_demo). or_oop_adj is the
// structural inverse — running `__lib_or_dsl_adj` per bit returns tmp to
// |0…0> after the post-swap-undo state where tmp == a | b.
//
...
//       readout matches the classical bitwise OR.
//   (3) or_oop followed by or_oop_adj returns tmp register to |0…0>.
//   (4) `sturm::invert<&sturm::detail::or_oop<W>>()` resolves to or_oop_adj
...
static void run_or_gate_path(uint32_t a_val, uint32_t b_val) {
    // Layout: a in [0..W), b in [W..2W), tmp allocated by or_oop in [2W..3W).
    // The gate-path lib_or_dsl emits CX+CX+CCX per bit on BitProxy, so no
    // ancilla is needed beyond the per-bit tmp allocation; n_orkan covers
    // 3*W up to W=3 plus headroom.
...

    {
        ...
        sturm::detail::or_oop_adj(a, b, tmp);
...
// invert<&or_oop<W>>() must resolve to or_oop_adj<W> at compile time.
static void test_invert_resolution() {
    constexpr auto adj_ptr =
        sturm::invert<&sturm::detail::or_oop<2>>();
    static_assert(adj_ptr != nullptr,
                  "invert<&or_oop<W>>() must resolve to a registered adjoint");
    static_assert(adj_ptr == &sturm::detail::or_oop_adj<2>,
                  "or_oop_adj must be the registered structural inverse");
    std::puts("  PASS: test_or_oop_invert_resolves_to_adj");
}

int main() {
    std::printf("sturm-ph6f.1 LO-2: or_oop reversible-network tests:\n");
    ...
    std::puts("  PASS: test_or_oop_classical_short_circuit");

    // Gate path -- W=2, all 16 (a,b) combinations.
    for (uint32_t a = 0; a < 4u; ++a)
        for (uint32_t b = 0; b < 4u; ++b)
            run_or_gate_path<2>(a, b);
    std::puts("  PASS: test_or_oop_gate_path_W2_full_matrix");

    // Gate path -- W=3, a couple of representative cases.
    run_or_gate_path<3>(0b101, 0b011);
    run_or_gate_path<3>(0b111, 0b111);
    run_or_gate_path<3>(0b000, 0b111);
    run_or_gate_path<3>(0b100, 0b010);
    std::puts("  PASS: test_or_oop_gate_path_W3");

    std::printf("All sturm-ph6f.1 tests passed.\n");
    return 0;
}
