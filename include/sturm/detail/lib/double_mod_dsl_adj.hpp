...
// starting from `x_bits = (2 · x_orig mod n_value)` (with the original
// `n_bits` preserved by the forward call), it returns `x_bits` to the
// original `x_orig` while leaving `n_bits` unchanged.  Implementation runs
// the forward gate sequence in reverse, swapping each `lib_add_dsl` for
// `lib_add_adj` (and vice-versa); the SWAP chain is reversed in iteration
// order; self-inverse XOR / `flip` / control-stack steps stay as-is.
//
...
//   restores `x_orig` ONLY when `x_bits` enters holding the value
//   produced by a paired `lib_double_mod_dsl` call on the same `n_bits`
//   operands.  Calling the adjoint on any other `x_bits` value produces
//   undefined output (the routine still emits well-formed gates, but
//   `x_bits` will be scrambled and `n_bits` may be corrupted).  This
//   matches the trust model documented in the forward header and the
//   PRD §5 contract; it is the resolution to issue sturm-wdas's design
//   note #3.
//
...
//          lt_flag.flip();   (reverse of forward's `lt_flag.flip()`)
//          lt_flag ^= x_bits[0];
...
//          lift_under(lt_flag) { lib_add_adj(n_ext, s_view, carry_anc, n+1); }
//   7'. Reverse forward step 4 (lib_add_adj on s_view):
//          lib_add_dsl(n_ext, s_view, lt_flag, n+1);
 ...
 *
 * Starting from `x_bits = (2 · x_orig) mod n_value` (with the original
 * `n_bits` preserved by the forward call), this routine returns `x_bits`
 * to `x_orig` while leaving `n_bits` unchanged.  Implementation runs the
 * forward gate sequence in reverse, swapping each `lib_add_dsl` for
 * `lib_add_adj` (and vice-versa); self-inverse XOR / `flip` / SWAP-XOR
 ...
 *               entry holds `(2 · x_orig) mod n_value` in bits 0..n-1
 *               and 0 in bit n.  On exit holds `x_orig` in bits 0..n-1
 *               and 0 in bit n.
 * @param n_bits Modulus register (n qubits, read but restored).
 * @param n      Register width.  `n == 0` short-circuits to a no-op.
 *
 * @pre `x_bits[0..n-1]` must enter holding the value produced by a
 *      paired `lib_double_mod_dsl` call on the same `n_bits` operands.
 *      `x_bits[n]` must be |0>.  `n_value` must be odd (matches the
 *      forward's precondition).  `x_bits` and `n_bits` must refer to
 *      physically distinct qubit registers.
 *
 * @par Behavior on precondition violation
 * The library does **not** check the precondition.  Calling
 * `__lib_double_mod_dsl_adj` with `x_bits` not equal to the paired
 * forward output, with `n_value` even, or with aliased registers is
 * **undefined behavior** — the routine still emits a well-formed gate
 * sequence, but `x_bits` will not be restored to the original value
 * and `n_bits` may be corrupted.  This matches the trust model shared
...
inline void __lib_double_mod_dsl_adj(Bit* x_bits, Bit* n_bits, std::size_t n) {
    ...
    //      receives a data-dependent comparison bit at step 7'; mark its
    ...
    // (3') Reverse forward step 8 (lt_flag uncompute):
    //         forward did `lt_flag ^= x_bits[0]; lt_flag.flip();`
    //      reverse runs `lt_flag.flip(); lt_flag ^= x_bits[0];`
    //      Result (for odd n_value): lt_flag is restored to (2x_orig < n).
    lt_flag.flip();
    lt_flag ^= x_bits[0];
    ...
    // (6') Reverse forward step 5 (controlled lib_add_dsl):
    //      controlled lib_add_adj on the same operands.  Same shared
    //      lift over `lt_flag_own` (the qbool anchor, super_mask=1).
    ...
    //      original 2x and clears lt_flag back to |0>.
    lib_add_dsl(n_ext, s_view, lt_flag, n + 1u);

    // (8') Release ancillas LIFO (carry_anc, lt_flag, n_pad).
